        subroutine NotAtALLNeeded
c       ARCCOM2e2.CH =filename                                                  
        IMPLICIT REAL*8 (A-H,M,O-Z)                                    
        PARAMETER (NMX=100,NMX2=2*NMX,NMX3=3*NMX,NMX4=4*NMX, 
     &  NMX8=8*NMX,NMXm=NMX*(NMX-1)) 
...
c           end if
           ...
           STOP
           END IF
           call FIND_CHAIN_INDICES
           IF(IWR.GT.0)WRITE(6,1232)time,(INAME(KW),KW=1,N)
           call INITIALIZE_XC_and_WC
           call CONSTANTS_OF_MOTION(ENERGY,G0,ALAG)
...
c           CMV(K)=0
           ...
           mmss=mmss+m(i)*m(j)
            ...
            call Take_Y_from_XC_WC(Y,Nvar)
            do i=1,Nvar
            SY(i)=0
            end do
            call Initial_Stepsize(X,V,M,N,ee,step) ! New initial step determination
            EPS=TOL
            Ncall=0
           END IF ! NEWREG
           KSTEPS=0
...
c           write(6,*)' step=',step,Nvar,stepi
           ...
           old=step
           call Initial_Stepsize(X,V,M,N,ee,step_new) ! New initial step determination
...
c--------------------------------------------------------
           ...
           call Put_Y_to_XC_WC(Y,Nvar) 
            call CHECK_SWITCHING_CONDITIONS(MUSTSWITCH)
            IF(MUSTSWITCH)THEN
            call Chain_Transformation !
...
c        if(1.e-3*mmij.gt.m(i)*m(j).and.cmethod(2).ne.0.0)then 
     ...
     & CMXinc(3),CMVinc(3),ENERGYinc,Energrinc,CHTIMEinc,spininc(3)
        COMMON/DerOfTime/G
        COMMON/DIAGNOSTICS/GAMMA,H,IWR
         save
        Te=-ENERGY-EnerGR
         if(cmethod(1).ne.0.0d0)then
        call EVALUATE_V(V,WC)
        do I=1,N
        I0=3*I-3
...
c       subroutine COORDINATE DEPENDENT PERTURBATIONS(ACC) ! USER DEFINED
       subroutine COORDINATE_DEPENDENT_PERTURBATI(ACC) ! USER DEFINED
        INCLUDE 'ARCCOM2e2.CH'
...
c                 ACC(ic0+k)=ACC(ic0+k)-MP(I)/rrr*dx(k) ! hope this is correct (not confirmed)
        ...
        subroutine Velocity_Dependent_Perturbations
...
c       h\E4iri\F6t        
        ...
        call Relativistic_ACCELERATIONS(dfr,dfGR,Va,spina,dspin)
              else
               do i=1,3*n
                dfr(i)=0
                 dfgr(i)=0
        ...
        SUBROUTINE CHECK_SWITCHING_CONDITIONS(MUSTSWITCH)
        INCLUDE 'ARCCOM2e2.CH'
        LOGICAL MUSTSWITCH
...
1        DO I=2,L
        LI=IND(I)
        IF( .NOT.USED(LI))THEN
        call CHECK_CONNECTION(IC,LMIN,LMAX,IJ,LI,SUC,LOOP)
        IF(SUC)THEN
        USED(LI)=.TRUE.
        GOTO 2
        ELSE
...
2        IF(LMAX-LMIN+1.LT.N)GO TO 1
        ...
        RETURN
        END
        SUBROUTINE CHECK_CONNECTION(IC,LMIN,LMAX,IJ,LI,SUC,LOOP)
         INCLUDE 'ARCCOM2e2.CH'
...
20      IF(j.le.ir)THEN
         ...
         GO TO 10
         END
        SUBROUTINE INITIALIZE_XC_AND_WC
        INCLUDE 'ARCCOM2e2.CH'
        save
C        Center of mass
        DO K=1,3
...
C        Rearrange according to INAME(i) and add CM.
        ...
        RETURN
        END
        SUBROUTINE CHAIN_TRANSFORMATION
        INCLUDE 'ARCCOM2e2.CH'
...
c        FY=min(FY,1.4) !1.4 ~ 1/0.7 ; where 0.7 = initial reduction factor
        ...
        save
        icollision=0
        call Put_Y_to_XC_WC(Y0,Nvar) ! Y -> XC, WTTL, WC
        call Initialize_increments_2_zero
        call  LEAPFROG(H,Leaps,stime) ! advance 
c        call Take Y from XC WC (Ytest,Nvars) ! take new results -> Y
        call take_increments_2_Y(y)
c        do i=1,Nvar
c        write(6,*)i,Y0(i)+Y(i)-Ytest(I),' i yNew-Yold '
c        end do
        return
        end
        subroutine Initialize_increments_2_zero
        INCLUDE 'ARCCOM2e2.CH'
        COMMON/IncrementCommon/WTTLinc,XCinc(NMX3),WCinc(NMX3),
     & CMXinc(3),CMVinc(3),ENERGYinc,Energrinc,CHTIMEinc,spininc(3)
        do i=1,3*(N-1)
        ...
        subroutine Take_Increments_2_Y(Y)
        INCLUDE 'ARCCOM2e2.CH'
        COMMON/IncrementCommon/WTTLinc,XCinc(NMX3),WCinc(NMX3),
     & CMXinc(3),CMVinc(3),ENERGYinc,Energrinc,CHTIMEinc,spininc(3)
...
c        Nvar=L  
        ...
        subroutine Put_Y_to_XC_WC(Y,Lmx)
         INCLUDE 'ARCCOM2e2.CH'
        real*8 Y(*)
        save
        L=1
        ...
        RETURN
        END
        subroutine Take_Y_from_XC_WC(Y,Nvar)
         INCLUDE 'ARCCOM2e2.CH'
        real*8 Y(*)
        save
        L=1
        ...
        RETURN
        END
        subroutine Obtain_Order_of_Y(SY)
        INCLUDE 'ARCCOM2e2.CH'
        real*8 SY(*)
        save
        w_old=0.1d0
...
C        Rearrange according to INAME(i) and add CM.
       ...
       SUBROUTINE Relativistic_ACCELERATIONS(ACC,ACCGR,Va,spina,dspin)
        INCLUDE 'ARCCOM2e2.CH'
...
c        dw(k)=dw(k)/(1+(vij2/cl**2)**2)**.25d0 ! not so good
        ...
        Ii=min(i,j)
        Jx=max(i,j)
         call Relativistic_Terms(Ii,dX,dW,rij,rdotv,vij2,
     & m(Ii),m(Jx),cl,DF,dfGR,spina,dsp)
            RS=2.d0*(m(i)+m(j))/CL**2
...
c        Grav.Rad.-terms
        ...
        END
         ...
         subroutine 
     & Relativistic_terms(I1,X,V,r,rdotv,v2,m1,m2,c,DV,DVgr,spina,dspin)
...
c  C.Will Q2-terms
     ...
     &  +1.5*Q2/r**4*(n(k)*(5*cdot(n,j)**2-1)-2*cdot(n,j)*j(k))
       ...
       return
        ...
        SUBROUTINE Initial_Stepsize(X,V,M,NB,ee,step)
        IMPLICIT real*8 (A-H,m,O-Z)
        DIMENSION X(*),V(*),M(*)
        save
        T=0.0
...
c     &  (1-7987*z/7095 + 54145*z*z/204336)
     ...
     &  (6*(1-7987*z/7095 + 54145*z*z/204336))
        ...
        SUBROUTINE CONSTANTS_OF_MOTION(ENE_NB,G,Alag)
...
c        mass=mass+M(I)
      ...
      SUBROUTINE FIND_BINARIES(time)  ! this is a toy analysis routine
      INCLUDE 'ARCCOM2e2.CH'
      REAL*8 XX(3),W(3)
...
c           call Coordinate Dependent Perturbations (acc) 
           call Coordinate_Dependent_Perturbati(acc) 
                 do i=1,n-1
                 do k=1,3
                 L=3*(i-1)
     ...
     &  ,spini(3),acc(*)
        save
        call EVALUATE_V(V,WCi) 
c adding V-dependent perts.
              if(clight.gt.0.0)then
              call Velocity_Dependent_Perturbations(dT,V,
     & spini,acc,dcmv,df,dfGR,dspin)
              else
              do i=1,3*n
              df(i)=acc(i)
              end do
     ...
     &  (V(I0+1)*GOM(k0+1)+V(I0+2)*GOM(K0+2)+V(I0+3)*GOM(K0+3))
        ...
        if(ind.eq.2)then
                do k=1,3
        spininc(k)=spininc(k)+dT*dspin(k)
        cmvinc(k)=cmvinc(k)+dT*dcmv(k)
        end do
        end if ! ind.eq.2
        RETURN
        END
