...
# for type in types:
	...
	for index in range(20):
		if True:
			...
			de2_flybys = np.array(de2_flybys)
			t = np.array(t)

			D1_tidal = de_abs_tidal[1:] / t[1:]
			D1_flybys = de_flybys[1:] / t[1:]
			D2 = de2_flybys[1:] / t[1:]
			D1D2 = D1_tidal / (D1_tidal + D2)
			...
			ax3.set_ylabel(r'$D_{1,2}$, ', fontsize=16)
			ax3.plot(t[1:], D1_tidal, 'r', label=r'$D_{\rm 1,tidal}$')
			ax3.plot(t[1:], D1_flybys, 'b', label=r'$D_{\rm 1,flybys}$')
			ax3.plot(t[1:], D2, 'k', label=r'$D_{\rm 2,flybys}$')
			...
			ax4.set_ylabel(r'$D_{\rm 1,tidal}/(D_{\rm 1,tidal}+D_{\rm 2,flybys})$', fontsize=16)
			ax4.plot(t[1:], D1D2, 'k')
			...
			pyplot.savefig(root_dir+"D1D2-"+type+"-"+str(index)+".pdf")
			pyplot.clf()