...
def third_body_cartesian(G,m,M_per,Q,e_per,theta_0):
    ...
    theta1 = np.arccos(-1.0/e_per)
    e_per_hat_vec = np.array([np.cos(theta1),np.sin(theta1),0.0])
    q_per_hat_vec = np.array([-np.sin(theta1),np.cos(theta1),0.0])
    j_per_hat_vec = np.array([0.0,0.0,1.0])
    for i in range(3):
...
def integrate(args):
    ...
    if args.do_nbody==True:
        nbody_a_sol,nbody_e_sol,nbody_i_sol,nbody_Delta_a,nbody_Delta_e,nbody_Delta_i,nbody_Delta_Omega,nbody_Delta_omega,nbody_energy_errors_sol = integrate_nbody(args)
        print( 'Q/a',args.Q/args.a)
        print( 'SA Delta e',Delta_e,'Delta i',Delta_i)
        print( 'nbody Delta e',nbody_Delta_e,'Delta i',nbody_Delta_i,'Delta Omega',nbody_Delta_Omega,'Delta omega',nbody_Delta_omega,'nbody energy error',nbody_energy_errors_sol)
...
def integrate1(args):
    ...
    # L_i = args.fraction_theta_i*np.arccos(-1.0/e_per)
    # L_f = args.fraction_theta_f*np.arccos(-1.0/e_per)

    # L_i = args.fraction_theta_i*np.pi
    # L_f = args.fraction_theta_f*np.pi
    ...
    N_steps = args.N_steps
    # thetas = np.linspace(-L, L, N_steps)
    thetas = np.linspace(L_i, L_f, N_steps)
    ODE_args = (eps_SA,eps_oct,e_per,args)
    RHR_vec = [ex,ey,ez,jx,jy,jz]
    ...
    if 1==0:
        ...
        # plot=fig.add_subplot(1,1,1)
        # plot.scatter(ex_sol,ey_sol)
        # plot.scatter(thetas, e_sol)
        # print(ex_sol)
        # pyplot.show()
        # pyplot.savefig('/home/alexander/Dropbox/code/flybys-master/e(theta).pdf')
        de = [e1-0.1 for e1 in e_sol]
        # print(de)
        a_per = Q/(e_per-1.0)
        r_array = a_per*(e_per**2-1)/(1+e_per*np.cos(thetas))/a
        file = open('/Users/axr6631/Dropbox/code/flybys-master/de(r)1_Q=30_hybrid.txt', "a")
        for member in np.transpose([r_array, de]):
            print(member[0], member[1], file=file)

    Delta_e = e_sol[-1] - e_sol[0]
    Delta_i = i_sol[-1] - i_sol[0]
    r, v = orbital_vectors_to_cartesian(args.G,args.m,args.a,0,ex_sol[-1],ey_sol[-1],ez_sol[-1],jx_sol[-1],jy_sol[-1],jz_sol[-1])
    a_fin, e_fin, i_fin, Omega_fin, omega_fin = orbital_elements_from_nbody(args.G,args.m,r,v)

    return a_fin, e_fin, i_fin, Omega_fin, omega_fin
...
def integrate_nbody(args):
    ...
    R3_max = 50 #radius that the perturber needs to escape with positive energy
    v_inf = 1
    p = 2
    ...
    a_per = G*(m+M_per)/v_inf**2
    e_per = np.sqrt(1.0+p**2/a_per**2)
    Q = a_per*(e_per-1.0)
    ...
    ex,ey,ez,jx,jy,jz = orbital_elements_to_orbital_vectors(e,i,omega,Omega)
    #vector from one binary component to the other
    ...
    theta_0 = -np.arccos((a_per/R3_max*(e_per**2-1)-1)/e_per)
    #3rd body coordinates in the binary reference frame 
    R3_rel,V3_rel = third_body_cartesian(G,m,M_per,Q,e_per,theta_0)

    #CoM coordinates in the binary reference frame
    ...
    #3rd body coordinates in the CoM reference frame
    R3 = -(m/(m+M_per))*R3_rel #= R3_rel - R_cm
    V3 = -(m/(m+M_per))*V3_rel
    ...
    tend = (1.0/n_per)*( -4*np.arctanh(((-1 + e_per)*np.tan(-theta_0/2.))/np.sqrt(-1 + e_per**2)) + (2*e_per*np.sqrt(-1 + e_per**2)*np.sin(-theta_0))/(1 + e_per*np.cos(-theta_0)) )
    ...
    E_per = 0  #perturber energy
    R3_sol = R3

    while not (E_per>0 and np.linalg.norm(R3_sol)>R3_max):
        sol = odeint(RHS_function_nbody, RHR_vec, times, args=ODE_args,mxstep=args.mxstep,rtol=1.0e-15)
        R1_sol = np.transpose(np.array([sol[:,0],sol[:,1],sol[:,2]]))[-1]
        V1_sol = np.transpose(np.array([sol[:,3],sol[:,4],sol[:,5]]))[-1]
        R2_sol = np.transpose(np.array([sol[:,6],sol[:,7],sol[:,8]]))[-1]
        V2_sol = np.transpose(np.array([sol[:,9],sol[:,10],sol[:,11]]))[-1]
        R3_sol = np.transpose(np.array([sol[:,12],sol[:,13],sol[:,14]]))[-1]
        V3_sol = np.transpose(np.array([sol[:,15],sol[:,16],sol[:,17]]))[-1]
        RHR_vec = [R1_sol[0],R1_sol[1],R1_sol[2],V1_sol[0],V1_sol[1],V1_sol[2],R2_sol[0],R2_sol[1],R2_sol[2],V2_sol[0],V2_sol[1],V2_sol[2],R3_sol[0],R3_sol[1],R3_sol[2],V3_sol[0],V3_sol[1],V3_sol[2]]
        times += tend
        E_per = np.linalg.norm(V3_sol)**2/2 - G*m/np.linalg.norm(R3_sol)
        print(times[-1], np.linalg.norm(R3_sol), E_per)
    ...
    a_sol,e_sol,i_sol,Omega_sol,omega_sol = orbital_elements_from_nbody(G,m1+m2,r_sol,v_sol)
    # a_sol.append(a)
    # e_sol.append(e)
    # i_sol.append(i)
    # Omega_sol.append(Omega)
    # omega_sol.append(omega)

    initial_energy = compute_total_energy(G,m1,m2,M_per,R1,V1,R2,V2,R3,V3)
    energy = compute_total_energy(G,m1,m2,M_per,R1_sol,V1_sol,R2_sol,V2_sol,R3_sol,V3_sol)
    energy_errors_sol = np.fabs((initial_energy-energy)/initial_energy) 
        
    Delta_a = a_sol - a
    Delta_e = e_sol - e
    Delta_i = i_sol - i
    Delta_Omega = Omega_sol - Omega
    Delta_omega = omega_sol - omega

    # print(Omega_sol[0],Omega_sol[1],Omega_sol[2])
    ...
    # plot2.plot(times,Omega_sol)
    # plot3.plot(times,omega_sol)
    # pyplot.show()
    # pyplot.savefig('G=0.1.pdf')

    print(R3, np.linalg.norm(V3_sol))

    return a_sol,e_sol,i_sol,Delta_a,Delta_e,Delta_i,Delta_Omega,Delta_omega,energy_errors_sol
