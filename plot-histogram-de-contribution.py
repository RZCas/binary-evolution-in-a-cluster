...
for type in types:
	...
	for index in range(0,10):
		...
		contributions = [0]*n
		binWidth = Qa_max/n
		binCentres = [Qa_max * (0.5+i)/n for i in range(n)] 
			...
			for line in f:
				...
				if len(data) > 1:
					...
					if lineNumber%3==0 and lineNumber>=6 and isfloat(data[1]):	#after the encounter
						...
						contributions[binNumber] += abs(e_f-e_i)

		contributions = np.array(contributions)
		contributions /= binWidth
		...
		pyplot.xlabel(r'$Q/a$')
		pyplot.ylabel(r'$d|\Delta e|/d(Q/a)$')
		pyplot.xscale('log')
		pyplot.yscale('log')
		plot_de.plot (binCentres, contributions, 'k')

		pyplot.tight_layout()
		pyplot.savefig(root_dir+"de-contibutions-"+type+"-"+str(index)+".pdf")
		pyplot.clf()