...
x_precise = []
lineNumber = 0
with open('output/outer_orbit.txt') as f:
	for line in f:
		data = line.split()
		t_precise.append(float(data[0]))
		x_precise.append(float(data[1]))
with open('output/outer_orbit_n=10.txt') as f:
	for line in f:
		data = line.split()
...
ax.set_ylabel(r'$x$ [pc]', fontsize=16)
# pyplot.xscale('log')
# pyplot.yscale('log')
...
pyplot.tight_layout()
pyplot.savefig('output/compare-orbits.pdf')